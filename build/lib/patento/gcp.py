...
def create_schema(dataframe):
    type_dict = {"int64":"INT", "float64":"FLO", "object":"STR"}

    for column_type, column_name, i in zip(dataframe.dtypes.values, dataframe.columns, range(len(dataframe.dtypes.values))):
        ...
        if i == 0:
            final_schema_str = "[\n"
            line_to_append = "bigquery.SchemaField('" + column_name + "', "+ type_dict[type_str] +", REQ, None, ()),\n"
        else: 
            line_to_append = "bigquery.SchemaField('" + column_name + "', "+ type_dict[type_str] +", NULL, None, ()),\n"

        final_schema_str = final_schema_str + line_to_append

        if i == len(dataframe.dtypes.values)-1:
            final_schema_str = final_schema_str + "]"
    print(final_schema_str)
